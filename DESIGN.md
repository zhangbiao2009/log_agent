...
flowchart TD
    ...
    L2 --> AGG["Aggregate (window, ~1 min)<br/>bucket by (service, patternID) → one Alert candidate per bucket"]
    AGG --> L3["L3: Anomaly Detection<br/>per-pattern rolling stats: spike / new pattern / rate jump"]
